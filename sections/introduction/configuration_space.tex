...
This section contains the definition of the configuration space $Y_W$ for a given Coxeter group $W$ and an example where we will show, for $W$ of type $A_n$, that $\pi_1(Y_W) \cong G_W$ and that $\pi_n(Y_W)$ is trivial for $n>1$ \cite{fox_neuwirth_braid_1962}.

For some finite or affine Coxeter group $W$ acting on $\R^n$, the set of reflections $R \in W$ acts on $\R^n$ by reflection through hyperplanes, one for each $r \in R$. For some $r \in R$ denote its hyperplane by $H(r) \subseteq \R^n$. Denote the union of all hyperplanes by $\mathcal{H} \coloneqq \bigcup_{r \in R} H(r)$.
...
With these tools in mind, we can then make our definition.
...
\begin{definition}
	...
	\end{equation*}
	and define the \emph{configuration space} $Y_W$ to be the quotient $Y/W$ with the action defined as above.
	\label{def:config_space}
\end{definition}

Note that the importance of $\C$ is that it is 2 dimensional. When one takes the complement of a co-dimension 1 object, you typically will not get any interesting topology. By complexifying the hyperplanes and then taking the complement within $\R^n \otimes \C$, we are effectively taking the complement of a codimension--2 object, and there is much more room for interesting topologies. The same construction can be achieved using $\R^{2n}$ and $\mathcal{H}\times\mathcal{H}$. A more general construction of $Y_W$ for all Coxeter groups using the \emph{Tits cone} can be found in \cite{paris_kpi1_2014}.
...
We can think of this as the space of $n+1$ distinct labelled points in $\C$, denoted $\conf_{n+1}(\C)$.

Historically, Artin \cite{artin_braids_1947} originally defined the braid group on $n$ strands $B_n$ to be $\pi_1(\conf_n(\R^2))$. He then proved the well known presentation of the braid group. 
