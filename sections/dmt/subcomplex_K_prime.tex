...
We will now define a subcomplex $K^\prime_W \subseteq K_W$ based on these fibre components, after which we will prove the necessary properties of $K^\prime_W$. Denote the power set of $\mathcal{F}(K_W)$ by $\mathcal{P}({\mathcal{F}(K)})$. Let $F\subseteq \mathcal{P}({\mathcal{F}(K)})$ be the set of all connected fibre components such that $\bigcup_{f \in F}f = \mathcal{F}(K_W)$. Recall  $X^\prime_W$ from \cref{def:subcomplex_X_prime}.
...
\begin{proof}
    ...
    $\bullet$ First we will prove the case where $\tau$ is in the top row of $f^\prime$ such that $x_0x_1\ldots x_{d-1} = w$. The two faces $[x_1|\cdots|x_{d-1}]$ and $[x_0|\cdots|x_{d-2}]$ are both already in $f^\prime$, so we need only check the faces
    \begin{equation*}
