...
\begin{remark}
    Each sequence $s \in S_d$ is uniquely determined by any length $(d-1)$ subsequence. If you have such a subsequence you are in the bottom row of \cref{fig:fibre_component_eg}. You may choose to move to the right along the Hasse diagram from $[x_1|\cdots|x_{d-1}]$ to $[x_1|\cdots|x_{d}]$. This is always possible and completely deterministic, since $x_1x_2\ldots x_{d-1}$ is a prefix of a minimal factorisation of $w$ and the product $x_1x_2\ldots x_d$ must equal $w$. Similarly, a movement up and leftward is always possible and deterministic using the balanced property of $K_W$. Any downward movement is always possible and obviously deterministic. Thus, a fibre component does not have initial of final nodes and its Hasse diagram will either be homeomorphic to $\R$ or $S^1$.
