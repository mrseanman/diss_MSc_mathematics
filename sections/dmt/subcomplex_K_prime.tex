...
\section{A New Subcomplex, \texorpdfstring{$K^\prime_W$}{K prime}}
...
\begin{proof}
    ...
    $\bullet$ First we will prove the case where $\tau$ is in the top row of the Hasse diagram for $f^\prime$ such that $x_0x_1\ldots x_{d-1} = w$. The two faces $[x_1|\cdots|x_{d-1}]$ and $[x_0|\cdots|x_{d-2}]$ are both already in $f^\prime$, so we need only check the faces
    \begin{equation*}
