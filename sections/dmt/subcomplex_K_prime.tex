...
\begin{figure}
    \begin{tikzpicture}[scale=1.2, baseline=0.5cm]
        ...
        \node[FSC]  (b5) at ($(b4) + (\hsep, -\vsep)$)      [label=below:{$\omega$}] {};
    ...
    \end{tikzpicture}
    \caption{The unique acyclic matching $\mathcal{M}_{f}$ on $f$ such that the critical cells correspond to exactly $f\prime$. Here $\alpha$ and $\omega$ correspond to the relevant first and last el}
\end{figure}
