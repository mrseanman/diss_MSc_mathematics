...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{Certain structures, if added to the fibre component, allow us to change orientation in a matching. Note that this is only demonstrative and no such fibre component exists.}
    \label{fig:example_orientation_switch}
\end{figure}

The simple form of the fibre components in \cref{chap:dmt} makes them easy to conceptualise, but we see that they lack enough structure to allow for the orientation change that we have shown is necessary. In the following section we propose a conceptually simple change to the construction in \cref{sec:subcomplex_K_prime}, that changes the poset map $\eta \colon \mathcal{F}(K_W) \to \N$ and will hopefully give us fibre components with enough structure to allow for orientation changes.
